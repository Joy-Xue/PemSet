...
# 百度说list最多存1152921504306846975=2^30个数，但是试了不行，这里设置list最多存2^31个数
...
def construct_D_file(Ct, k, m, r, g):
    ...
    # D = np.zeros(l)
    # 将D存入文件中
    filename = '../../temp/pckv_ue/D' + str(r) + '.txt'
    f1 = open(filename, 'w')
    # p+1应该比i*sl+j计算的快吧，用p来指示到哪一位了
    # p = 0
    for i in range(k):
...
if __name__ == '__main__':
    ...
    # f1_name = '../../result/synthetic/epsilon/f1_pckv_ue_g60_c5_epsilon.txt'
    # ncr_name = '../../result/synthetic/epsilon/ncr_pckv_ue_g60_c5_epsilon.txt'
    f1_name = '../../result/synthetic/f1_pckv_ue_g6_c5_epsilon.txt'
    ncr_name = '../../result/synthetic/ncr_pckv_ue_g6_c5_epsilon.txt'
    file_f1 = open(f1_name, 'w')
    file_ncr = open(ncr_name, 'w')
    # ct_name = '../../result/synthetic/epsilon/ct_pckv_ue_g60_c5_epsilon.txt'
    # dist_name = '../../result/synthetic/epsilon/dist_pckv_ue_g60_c5_epsilon.txt'
    ct_name = '../../result/synthetic/ct_pckv_ue_g6_c5_epsilon.txt'
    dist_name = '../../result/synthetic/dist_pckv_ue_g6_c5_epsilon.txt'
    ...
    for t_num_i in range(t_num):
        ...
        for i in range(num):
            ...
            print(i, 'f1', pckv_ue_f1[i], 'ncr', pckv_ue_ncr[i])
            print(PCKV_UE_Ct)
            print(PCKV_UE_dist)
            # print(i, 'ncr', pckv_ue_ncr[i])
            file_f1.write(str(pckv_ue_f1[i]) + ' ')
            file_ncr.write(str(pckv_ue_ncr[i]) + ' ')
            for t_file in range(k):
                file_ct.write(str(PCKV_UE_Ct[t_file]) + ' ')
                file_dist.write(str(PCKV_UE_dist[t_file]) + ' ')
            file_ct.write('\n')
            file_dist.write('\n')
        file_f1.write('\n')
        file_ncr.write('\n')
