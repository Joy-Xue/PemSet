...
from set_wheel_flatten import set_wheel_flatten, set_wheel_file_flatten
from read_data_dist import read_data
from cal_flatten_num import cal_r_flatten_num
from set_wheel_flatten_frequency import set_wheel_flatten_frequency, set_wheel_file_flatten_frequency
from set_wheel_frequency import set_wheel_frequency, set_wheel_file_frequency
    ...
    for i in range(1, g + 1):
        ...
        for j in range(r_start, r_stop):
            for t_j in range(c):
                ...
                # print(X[i][j])
                prefix_X[j][t_j] = int('{:064b}'.format(int(X[j][t_j]))[0:s], 2)
    return prefix_X, si
...
def construct_D_file(Ct, k, m, r, g):
    ...
    # D = np.zeros(l)
    # 将D存入文件中
    filename = '../../temp/wheel_flatten_frequency/D' + str(r) + '.txt'
    f1 = open(filename, 'w')
    # p+1应该比i*sl+j计算的快吧，用p来指示到哪一位了
    # p = 0
    for i in range(k):
...
# t_blend轮之前用flatten，之后用普通的，所以t_blend = 1, 2, 3, ..., g + 1
# 修改为根据每轮的情况来判断flatten的次数
def PEM_wheel_flatten_frequency(prefix_X, k, m, g, c, epsilon, N, si):
    ...
    if l_s < 33:
        for i in range(1, g + 1):
            ...
            flatten_num = cal_r_flatten_num(m, k, c, g, i)
            if i == 1:
                Ct = C0
                D = construct_D(Ct, k, m, i, g)
                if flatten_num == 1:
                    EstimateDist_wheel_flatten_frequency = set_wheel_frequency(t_X, Ni, c, epsilon, D)
                else:
                    s = pow(2, si[i])
                    EstimateDist_wheel_flatten_frequency = set_wheel_flatten_frequency(t_X, Ni, c, epsilon, D, s, flatten_num)
                Ct, wheel_flatten_frequency_dist = construct_C(EstimateDist_wheel_flatten_frequency, k, D)
            elif i == g:
                D = construct_D(Ct, k, m, i, g)
                EstimateDist_wheel_flatten_frequency = set_wheel_frequency(t_X, Ni, c, epsilon, D)
                Ct, wheel_flatten_frequency_dist = construct_C(EstimateDist_wheel_flatten_frequency, k, D)
        ...
        for i in range(1, g + 1):
            ...
            if i == 1:
                ...
                if flatten_num == 1:
                    set_wheel_file_frequency(t_X, Ni, c, epsilon, i)
                else:
                    s = pow(2, si[i])
                    set_wheel_file_flatten_frequency(t_X, Ni, c, epsilon, i, s, flatten_num)
                Ct, wheel_flatten_frequency_dist = construct_C_file(k, i)
            elif i == g:
                construct_D_file(Ct, k, m, i, g)
                set_wheel_file_frequency(t_X, Ni, c, epsilon, i)
                Ct, wheel_flatten_frequency_dist = construct_C_file(k, i)
                ...
                if flatten_num == 1:
                    ...
                    set_wheel_file_flatten_frequency(t_X, Ni, c, epsilon, i, s, flatten_num)
                Ct, wheel_flatten_frequency_dist = construct_C_file(k, i)
    return Ct, wheel_flatten_frequency_dist
...
if __name__ == '__main__':
    ...
    file_name = '../../data/synthetic_data/64_10.txt'
    X, Real_k_dist, Real_k_data, N, c = read_data(file_name, k)
    print(Real_k_data)
    print(Real_k_dist)
    prefix_X, si = data_to_prefix(X, N, g, c, m, k)
    ct_name = '../../result/synthetic/epsilon/ct_wheel_flatten_frequency_g6_c10_epsilon.txt'
    dist_name = '../../result/synthetic/epsilon/dist_wheel_flatten_frequency_g6_c10_epsilon.txt'
    f1_name = '../../result/synthetic/epsilon/f1_wheel_flatten_frequency_g6_c10_epsilon.txt'
    ncr_name = '../../result/synthetic/epsilon/ncr_wheel_flatten_frequency_g6_c10_epsilon.txt'
    ...
    for t_num_i in range(t_num):
        ...
        # t_blend [1, 7]
        # t_blend = t_num_i + 1
        wheel_flatten_frequency_f1 = [0 for col in range(num)]
        wheel_flatten_frequency_ncr = [0 for col in range(num)]
        for i in range(num):
            epsilon = (i + 1) * 0.5
            wheel_flatten_frequency_Ct, wheel_flatten_frequency_dist = PEM_wheel_flatten_frequency(prefix_X, k, m, g, c, epsilon, N, si)
            wheel_flatten_frequency_f1[i] = cal_f1(Real_k_data, wheel_flatten_frequency_Ct)
            wheel_flatten_frequency_ncr[i] = cal_ncr(Real_k_data, wheel_flatten_frequency_Ct, Real_k_dist, wheel_flatten_frequency_dist, k)
            # print(i, 'f1', wheel_flatten_frequency_f1[i])
            # print(i, 'ncr', wheel_flatten_frequency_ncr[i])
            print(i, 'f1', wheel_flatten_frequency_f1[i], 'ncr', wheel_flatten_frequency_ncr[i])
            print(wheel_flatten_frequency_Ct)
            print(wheel_flatten_frequency_dist)
            file_f1.write(str(wheel_flatten_frequency_f1[i]) + ' ')
            file_ncr.write(str(wheel_flatten_frequency_ncr[i]) + ' ')
            for t_file in range(k):
                file_ct.write(str(wheel_flatten_frequency_Ct[t_file]) + ' ')
                file_dist.write(str(wheel_flatten_frequency_dist[t_file]) + ' ')
            file_ct.write('\n')
            file_dist.write('\n')
        file_f1.write('\n')
        file_ncr.write('\n')
        print(t_num_i)
        print(wheel_flatten_frequency_f1)
        print(wheel_flatten_frequency_ncr)
    file_f1.close()
    file_ncr.close()
    file_ct.close()
    file_dist.close()
