...
def construct_D_file(Ct, k, m, r, g):
    ...
    # D = np.zeros(l)
    # 将D存入文件中
    filename = '../../temp/wheel_flatten/D' + str(r) + '.txt'
    f1 = open(filename, 'w')
    # p+1应该比i*sl+j计算的快吧，用p来指示到哪一位了
    # p = 0
    for i in range(k):
...
def PEM_wheel_flatten(prefix_X, k, m, g, c, epsilon, N, si):
    S0 = math.ceil(math.log2(k))
    C0 = [x for x in range(0, pow(2, S0))]
    l_s = math.ceil(math.log2(k)) + math.ceil((m - math.ceil(math.log2(k))) / g)
    if l_s < 33:
        for i in range(1, g + 1):
            ...
            if i == 1:
                ...
                D = construct_D(Ct, k, m, i, g)
                if flatten_num == 1:
                    EstimateDist_wheel_flatten = set_wheel(t_X, Ni, c, epsilon, D)
                else:
                    s = pow(2, si[i])
                    EstimateDist_wheel_flatten = set_wheel_flatten(t_X, Ni, c, epsilon, D, s, flatten_num)
                Ct, wheel_flatten_dist = construct_C(EstimateDist_wheel_flatten, k, D)
            elif i == g:
                D = construct_D(Ct, k, m, i, g)
                EstimateDist_wheel_flatten = set_wheel(t_X, Ni, c, epsilon, D)
                Ct, wheel_flatten_dist = construct_C(EstimateDist_wheel_flatten, k, D)
        ...
        for i in range(1, g + 1):
            ...
            if i == 1:
                ...
                if flatten_num == 1:
                    ...
                    s = pow(2, si[i])
                    set_wheel_file_flatten(t_X, Ni, c, epsilon, i, s, flatten_num)
                Ct, wheel_flatten_dist = construct_C_file(k, i)
            elif i == g:
                construct_D_file(Ct, k, m, i, g)
                set_wheel_file(t_X, Ni, c, epsilon, i)
                Ct, wheel_flatten_dist = construct_C_file(k, i)
...
if __name__ == '__main__':
    ...
    file_name = '../../data/synthetic_data/64_5.txt'
    X, Real_k_dist, Real_k_data, N, c = read_data(file_name, k)
    print(Real_k_data)
    print(Real_k_dist)
    prefix_X, si = data_to_prefix(X, N, g, c, m, k)
    f1_name = '../../result/synthetic/epsilon/f1_wheel_flatten_g6_c5_epsilon.txt'
    ncr_name = '../../result/synthetic/epsilon/ncr_wheel_flatten_g6_c5_epsilon.txt'
    file_f1 = open(f1_name, 'a')
    file_ncr = open(ncr_name, 'a')
    ct_name = '../../result/synthetic/epsilon/ct_wheel_flatten_g6_c5_epsilon.txt'
    dist_name = '../../result/synthetic/epsilon/dist_wheel_flatten_g6_c5_epsilon.txt'
    file_ct = open(ct_name, 'a')
    file_dist = open(dist_name, 'a')
    t_num = 5
    for t_num_i in range(t_num):
        num = 10
        # t_blend [1, 7]
        # t_blend = t_num_i + 1
        wheel_flatten_f1 = [0 for col in range(num)]
        wheel_flatten_ncr = [0 for col in range(num)]
        for i in range(num):
            epsilon = (i + 1) * 0.5
            wheel_flatten_Ct, wheel_flatten_dist = PEM_wheel_flatten(prefix_X, k, m, g, c, epsilon, N, si)
            wheel_flatten_f1[i] = cal_f1(Real_k_data, wheel_flatten_Ct)
            wheel_flatten_ncr[i] = cal_ncr(Real_k_data, wheel_flatten_Ct, Real_k_dist, wheel_flatten_dist, k)
            # print(i, 'f1', wheel_flatten_f1[i])
            # print(i, 'ncr', wheel_flatten_ncr[i])
            print(i, 'f1', wheel_flatten_f1[i], 'ncr', wheel_flatten_ncr[i])
            print(wheel_flatten_Ct)
            print(wheel_flatten_dist)
            file_f1.write(str(wheel_flatten_f1[i]) + ' ')
            file_ncr.write(str(wheel_flatten_ncr[i]) + ' ')
            for t_file in range(k):
                file_ct.write(str(wheel_flatten_Ct[t_file]) + ' ')
                file_dist.write(str(wheel_flatten_dist[t_file]) + ' ')
            file_ct.write('\n')
            file_dist.write('\n')
        file_f1.write('\n')
        file_ncr.write('\n')
        print(t_num_i)
        print(wheel_flatten_f1)
        print(wheel_flatten_ncr)
    file_f1.close()
    file_ncr.close()
    file_dist.close()
    file_ct.close()
