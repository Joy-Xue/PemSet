...
def construct_C_file(k, r):
    ...
    top_k_data = [0 for col in range(k)]
    top_k_dist = [0 for col in range(k)]
    file_d_name = 'temp/D' + str(r) + '.txt'
    file_dist_name = 'temp/dist' + str(r) + '.txt'
...
if __name__ == '__main__':
    ...
    num = 10
    pckv_ue_f1 = [0 for col in range(num)]
    pckv_ue_ncr = [0 for col in range(num)]
    olh_f1 = [0 for col in range(num)]
    olh_ncr = [0 for col in range(num)]
    olh_sample_f1 = [0 for col in range(num)]
    olh_sample_ncr = [0 for col in range(num)]
    wheel_f1 = [0 for col in range(num)]
    wheel_ncr = [0 for col in range(num)]
    f1_name = 'result/f1.txt'
    ncr_name = 'result/ncr.txt'
    file_f1 = open(f1_name, 'w')
    file_ncr = open(ncr_name, 'w')
    for i in range(num):
        epsilon = (i + 1) * 0.5
        PCKV_UE_Ct, PCKV_UE_dist = PEM_PCKV_UE(prefix_X, k, m, g, c, epsilon, N, si)
        pckv_ue_f1[i] = cal_f1(Real_k_data, PCKV_UE_Ct)
        pckv_ue_ncr[i] = cal_ncr(Real_k_data, PCKV_UE_Ct, Real_k_dist, PCKV_UE_dist, k)
        OLH_Ct, OLH_dist = PEM_OLH(prefix_X, k, m, g, c, epsilon, N)
        olh_f1[i] = cal_f1(Real_k_data, OLH_Ct)
        olh_ncr[i] = cal_ncr(Real_k_data, OLH_Ct, Real_k_dist, OLH_dist, k)
        ...
        wheel_f1[i] = cal_f1(Real_k_data, wheel_Ct)
        wheel_ncr[i] = cal_ncr(Real_k_data, wheel_Ct, Real_k_dist, wheel_dist, k)
        print(i, 'f1', pckv_ue_f1[i], olh_f1[i], olh_sample_f1[i], wheel_f1[i])
        print(i, 'ncr', pckv_ue_ncr[i], olh_ncr[i], olh_sample_ncr[i], wheel_ncr[i])
        file_f1.write(str(pckv_ue_f1[i]) + ' ' + str(olh_f1[i]) + ' ' + str(olh_sample_f1[i]) + ' ' + str(wheel_f1[i]) + '\n')
        file_ncr.write(str(pckv_ue_ncr[i]) + ' ' + str(olh_ncr[i]) + ' ' + str(olh_sample_ncr[i]) + ' ' + str(wheel_ncr[i]) + '\n')
    file_f1.close()
    file_ncr.close()
    print('PCKV_UE')
    print(pckv_ue_f1)
    ...
    plt.figure(2)
    plt.subplot(121)
    plt.plot(range(num), pckv_ue_f1, 'r', label='PCKV_UE')
    plt.plot(range(num), olh_f1, 'b', label='OLH')
    plt.plot(range(num), olh_sample_f1, 'g', label='PLH_sample')
    plt.plot(range(num), wheel_f1, 'y', label='Wheel')
    plt.legend(loc='best')
    plt.xlabel('$\epsilon$')
    plt.ylabel('F1')
    plt.subplot(122)
    plt.plot(range(num), pckv_ue_ncr, 'r', label='PCKV_UE')
    plt.plot(range(num), olh_ncr, 'b', label='OLH')
    plt.plot(range(num), olh_sample_ncr, 'g', label='PLH_sample')
    plt.plot(range(num), wheel_ncr, 'y', label='Wheel')
    plt.legend(loc='best')
    plt.xlabel('$\epsilon$')
    plt.ylabel('NCR')
    plt.show()
