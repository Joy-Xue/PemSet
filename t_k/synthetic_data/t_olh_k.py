...
def cal_var(ct, cg, ct_dist, cg_dist, k):
    num = 0
    var = 0.0
    for i in range(k):
        if ct[i] in cg:
            num += 1
            var += (ct_dist[i] - cg_dist[i]) * (ct_dist[i] - cg_dist[i])
    if num != 0:
        return var / num
    else:
        return 0.0
...
if __name__ == '__main__':
    ...
    f1_name = '../../result/synthetic/k/f1_olh_g6_k.txt'
    ncr_name = '../../result/synthetic/k/ncr_olh_g6_k.txt'
    var_name = '../../result/synthetic/k/var_olh_g6_k.txt'
    file_f1 = open(f1_name, 'a')
    file_ncr = open(ncr_name, 'a')
    file_var = open(var_name, 'a')
    ct_name = '../../result/synthetic/k/ct_olh_g6_k.txt'
    dist_name = '../../result/synthetic/k/dist_olh_g6_k.txt'
    ...
    for t_num_i in range(t_num):
        ...
        olh_var = [0 for col in range(num)]
        for i in range(num):
            k = i * 2 + 1
            ...
            olh_var[i] = cal_var(Real_k_data, olh_Ct, Real_k_dist, olh_dist, k)
            # print(i, 'f1', olh_f1[i])
            # print(i, 'ncr', olh_ncr[i])
            print(k, 'f1', olh_f1[i], 'ncr', olh_ncr[i])
            print(olh_Ct)
            print(olh_dist)
            file_f1.write(str(olh_f1[i]) + ' ')
            file_ncr.write(str(olh_ncr[i]) + ' ')
            file_var.write(str(olh_var[i]) + ' ')
            for t_file in range(k):
                file_ct.write(str(olh_Ct[t_file]) + ' ')
                file_dist.write(str(olh_dist[t_file]) + ' ')
            file_ct.write('\n')
            file_dist.write('\n')
            file_ct.flush()
            file_dist.flush()
